...
public class SummaryInputList extends MarketInputFolderList implements IBusinessObjectList
{
  
  public SummaryInputList() throws OculusException
  {
    super();
  }

  protected SummaryInputList(Comparator sortCrit) throws OculusException
  {
    super (sortCrit);
  }
  ...
  { 
    ...
    return "SELECT "
          ...
          +" WHERE mi.BASECLASSID="+IDCONST.SUMMARYINPUT.getIIDValue()
          +" AND fil.FOLDERID="+getIID()+ " AND fil.DELETESTATE<>"+DeleteState.DELETED.getIntValue()+" AND mi.DELETESTATE<>"+DeleteState.DELETED.getIntValue();
  ...
  {
    SummaryInputList sortedList = new SummaryInputList(sortCrit);
...
//      CRM.getInstance().returnDatabaseConnection(repConn);
  ...
  {
    SummaryInputList featList = null;
      featList = new SummaryInputList();
