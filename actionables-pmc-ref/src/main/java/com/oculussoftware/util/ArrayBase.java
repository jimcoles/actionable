/*************************
A utility that simulates a dynamic array. Use this if you want to store variable number
of a primitive type in some container other than a Vector/ArrayList.

Benefits: 1. You don't have to cast the freakin' primitive into an object before storing it.
...
public abstract class ArrayBase
{
    ...
    protected ArrayBase(int size, int growth, Class type) {
    ...
    protected ArrayBase(int size, Class type) {
    ...
    protected abstract void removeValues(int from, int to);
        ...
        if (index >= 0 && index < countPresent) {
            if (index < --countPresent){
                ...
                System.arraycopy(base, index + 1, base, index,
                    countPresent - index);
                removeValues(countPresent, countPresent + 1);
            }
        } else {
            throw new ArrayIndexOutOfBoundsException("Invalid index value");
        }
    ...
    public void setSize(int count) {
        if (count > countLimit) {
            ...
            removeValues(count, countPresent);
        }
        countPresent = count;
    }
