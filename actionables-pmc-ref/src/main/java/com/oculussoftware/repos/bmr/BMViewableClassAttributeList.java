...
public class BMViewableClassAttributeList extends BMModelElementList 
{
  private IIID _user = null;
  protected ArrayList _perms;
  protected Iterator _permIds;
   
 public BMViewableClassAttributeList() throws OculusException
  {
    _guid = new GUID();
    _coll = new ArrayList();
    ...
    throws OculusException
  {
    return " SELECT attrib.OBJECTID,caga.ORDERNUM,MIN(agg.OPERATIONTYPE) AS OPERATIONTYPE"+
           ...
           " WHERE caga.CLASSID = "+getIID()+" AND (1="+_user+" OR ((agg.OPERATIONTYPE = "+AttrGroupOper.VIEW+" OR agg.OPERATIONTYPE = "+AttrGroupOper.EDIT+") AND "+
  ...
  { 
    BMViewableClassAttributeList gColl = new BMViewableClassAttributeList();
    gColl.setIID(_iid);
