...
public class BMEditableAttributeList extends BMModelElementList 
{
  private IIID _user = null;
  protected ArrayList _perms;
  protected Iterator _permIds;
   
 public BMEditableAttributeList() throws OculusException
  {
    _guid = new GUID();
    _coll = new ArrayList();
    ...
    throws OculusException
  {
    return	" SELECT attrib.OBJECTID,caga.ORDERNUM,MIN(agg.OPERATIONTYPE) AS OPERATIONTYPE"+
            ...
            "  (1="+_user+" OR (agg.OPERATIONTYPE="+AttrGroupOper.EDIT+" AND (agg.ACCESSORID="+_user+" OR (agg.ACCESSORID IN (SELECT ug.GROUPID FROM USERGROUPASC ug WHERE ug.USERID="+_user+"))))) "+
  ...
  { 
    BMEditableAttributeList gColl = new BMEditableAttributeList();
    gColl.setIID(_iid);
