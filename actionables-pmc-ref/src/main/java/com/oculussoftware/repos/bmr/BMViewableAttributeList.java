...
public class BMViewableAttributeList extends BMModelElementList 
{
  private IIID _user = null;
  protected ArrayList _perms;
  protected Iterator _permIds;
   
 public BMViewableAttributeList() throws OculusException
  {
    _guid = new GUID();
    _coll = new ArrayList();
    ...
    throws OculusException
  {
    return " SELECT attrib.OBJECTID, caga.ORDERNUM, MIN(agg.OPERATIONTYPE) AS OPERATIONTYPE "+
           " FROM CLASSATTRGROUPASC caga LEFT OUTER JOIN ENTRYFORM form ON  form.FORMCLASSID = caga.CLASSID OR form.SCNDCLASSID = caga.CLASSID "+
           ...
           " WHERE form.OBJECTID = "+getIID()+" AND (1="+_user+" OR ((agg.OPERATIONTYPE = "+AttrGroupOper.VIEW+" OR agg.OPERATIONTYPE = "+AttrGroupOper.EDIT+") AND "+
           "  (agg.ACCESSORID = "+_user+" OR agg.ACCESSORID IN "+
           "    (SELECT ug.GROUPID FROM USERGROUPASC ug  WHERE ug.USERID = "+_user+")))) "+
           " GROUP BY attrib.OBJECTID, attrib.NAME, caga.ORDERNUM "+
           " ORDER BY 2";
  } 
    
  public Object dolly() throws OculusException 
  { 
    BMViewableAttributeList gColl = new BMViewableAttributeList();
    gColl.setIID(_iid);
